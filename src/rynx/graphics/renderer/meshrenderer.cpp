...
rynx::graphics::renderer::renderer(
  ...
  m_pTextRenderer =
      rynx::make_unique<rynx::graphics::text_renderer>(textures, shaders);
...
void rynx::graphics::renderer::init() {
  ...
  {
    shader_single =
        m_shaders->load_shader("renderer2d", "/shaders/geometry_shader.vs.glsl",
                               "/shaders/geometry_shader.fs.glsl");
    shader_instanced = m_shaders->load_shader(
        "renderer2d_instanced", "/shaders/geometry_shader_instanced.vs.glsl",
        "/shaders/geometry_shader_instanced.fs.glsl");
    shader_instanced_deferred = m_shaders->load_shader(
        "renderer2d_instanced_deferred",
        "/shaders/geometry_shader_instanced_deferred.vs.glsl",
        "/shaders/geometry_shader_instanced_deferred.fs.glsl");

    shader_single->activate();
    shader_single->uniform("tex", 0);
    shader_single->uniform("uvIndirect", 1);
    shader_single->uniform("atlasBlocksPerRow",
                           m_textures->getAtlasBlocksPerRow());

    shader_instanced->activate();
    shader_instanced->uniform("tex", 0);
    shader_instanced->uniform("uvIndirect", 1);
    shader_instanced->uniform("atlasBlocksPerRow",
                              m_textures->getAtlasBlocksPerRow());

    shader_instanced_deferred->activate();
    shader_instanced_deferred->uniform("tex", 0);
    shader_instanced_deferred->uniform("uvIndirect", 1);
    shader_instanced_deferred->uniform("atlasBlocksPerRow",
                                       m_textures->getAtlasBlocksPerRow());
  }

  {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 InstancesPerDrawCall * 4 * 4 * sizeof(GLfloat), NULL,
                 GL_STREAM_DRAW);

    glGenBuffers(1, &colors_buffer);
    glBindBuffer(GL_ARRAY_BUFFER, colors_buffer);
    glBufferData(GL_ARRAY_BUFFER, InstancesPerDrawCall * 4 * sizeof(GLfloat),
                 NULL, GL_STREAM_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, InstancesPerDrawCall * 1 * sizeof(GLint),
                 NULL, GL_STREAM_DRAW);

    glGenBuffers(1, &opaqueness_buffer);
    glBindBuffer(GL_ARRAY_BUFFER, opaqueness_buffer);
    glBufferData(GL_ARRAY_BUFFER, InstancesPerDrawCall * 1 * sizeof(GLubyte),
                 NULL, GL_STREAM_DRAW);
  }
...
void rynx::graphics::renderer::drawLine(const vec3<float> &p1,
                                        const vec3<float> &p2, float width,
                                        const floats4 &color) {
  matrix4 model;
  model.identity();
...
void rynx::graphics::renderer::drawLine(const vec3<float> &p1,
                                        const vec3<float> &p2,
                                        const matrix4 &model, float width,
                                        const floats4 &color) {
...
void rynx::graphics::renderer::drawRectangle(const matrix4 &model,
...
void rynx::graphics::renderer::drawMesh(const rynx::graphics::mesh &mesh,
...
void rynx::graphics::renderer::drawText(
    const rynx::graphics::renderable_text &text) {
  m_pTextRenderer->drawText(text);
}

void rynx::graphics::renderer::instanced_draw_impl(
    const rynx::graphics::mesh &mesh, size_t num_instances,
    const matrix4 *models, const floats4 *colors,
    const rynx::graphics::texture_id *tex_ids, const uint8_t *opaqueness,
    DrawType type) {
...

  {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, model_matrices_buffer);
    for (int i = 0; i < 4; ++i) {
      glVertexAttribPointer(model_matrix_slot + i, 4, GL_FLOAT, GL_FALSE,
                            sizeof(GLfloat) * 4 * 4,
                            (void *)(i * 4 * sizeof(float)));
      glEnableVertexAttribArray(model_matrix_slot + i);
      glVertexAttribDivisor(model_matrix_slot + i,
                            1); // model matrices, one per entity -> 1
    }
  }
...

  {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, colors_buffer);
    glVertexAttribPointer(colors_slot, 4, GL_FLOAT, GL_FALSE,
                          sizeof(GLfloat) * 4, 0);
    glEnableVertexAttribArray(colors_slot);
    glVertexAttribDivisor(colors_slot, 1); // colors, use one per entity -> 1
  }

  const GLint opaqueness_slot = shader->attribute_optional("opaqueness");
  if (opaqueness_slot >= 0) {
    glBindBuffer(GL_ARRAY_BUFFER, opaqueness_buffer);
    // opaqueness is a byte. To pass it correctly (since the shader might
    // expect float or uint), let's interpret it as normalized float for GLSL
    glVertexAttribPointer(opaqueness_slot, 1, GL_UNSIGNED_BYTE, GL_TRUE,
                          sizeof(GLubyte), 0);
    glEnableVertexAttribArray(opaqueness_slot);
    glVertexAttribDivisor(opaqueness_slot, 1);
  }
  ...
  while (num_instances > currentIteration * InstancesPerDrawCall) {
    int32_t remaining = static_cast<int32_t>(
        num_instances - currentIteration * InstancesPerDrawCall);
    int32_t instances_for_current_iteration =
        remaining > InstancesPerDrawCall ? InstancesPerDrawCall : remaining;

    {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0,
                      instances_for_current_iteration * 4 * 4 * sizeof(GLfloat),
                      models + currentIteration * InstancesPerDrawCall);
    }

    {
      ...
      glBufferData(GL_ARRAY_BUFFER, InstancesPerDrawCall * 4 * sizeof(GLfloat),
                   NULL, GL_STREAM_DRAW);
      glBufferSubData(GL_ARRAY_BUFFER, 0,
                      instances_for_current_iteration * 4 * sizeof(GLfloat),
                      colors + currentIteration * InstancesPerDrawCall);
    }

    {
      ...
      glBufferData(GL_ARRAY_BUFFER, InstancesPerDrawCall * 1 * sizeof(GLint),
                   NULL, GL_STREAM_DRAW);
      glBufferSubData(GL_ARRAY_BUFFER, 0,
                      instances_for_current_iteration * 1 * sizeof(GLint),
                      tex_ids + currentIteration * InstancesPerDrawCall);
    }
    ...
    if (opaqueness_slot >= 0) {
      ...
      glBufferData(GL_ARRAY_BUFFER, InstancesPerDrawCall * 1 * sizeof(GLubyte),
                   NULL, GL_STREAM_DRAW);
      if (opaqueness != nullptr) {
        glBufferSubData(GL_ARRAY_BUFFER, 0,
                        instances_for_current_iteration * 1 * sizeof(GLubyte),
                        opaqueness + currentIteration * InstancesPerDrawCall);
      } else {
        // fill an array of 128s so we can write default value for elements
        // without opaqueness
        static std::vector<GLubyte> default_opaqueness(InstancesPerDrawCall,
                                                       128);
        glBufferSubData(GL_ARRAY_BUFFER, 0,
                        instances_for_current_iteration * 1 * sizeof(GLubyte),
                        (const GLvoid *)default_opaqueness.data());
      }
    }
    ...
    glDrawElementsInstanced(mode, mesh.getIndexCount(), GL_UNSIGNED_SHORT, 0,
                            instances_for_current_iteration);
    ++currentIteration;
  }

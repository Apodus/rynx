...
class GraphicsDLL renderer {
  ...
  GLuint opaqueness_buffer = 0;

  bool verifyNoGLErrors() const;

  enum class DrawType { Forward, Deferred };

  void instanced_draw_impl(
      const mesh &mesh, size_t num_instances, const matrix4 *models,
      const floats4 *colors, const rynx::graphics::texture_id *tex_ids,
      const uint8_t *opaqueness, DrawType type);

public:
  renderer(rynx::shared_ptr<rynx::graphics::GPUTextures> texture,
  ...
  void setDefaultFont(const Font &font) {
    m_pTextRenderer->setDefaultFont(font);
  }
  const Font &getDefaultFont() const {
    return m_pTextRenderer->getDefaultFont();
  }

  void drawLine(const vec3<float> &p1, const vec3<float> &p2, float width,
                const floats4 &color);
  void drawLine(const vec3<float> &p1, const vec3<float> &p2,
                const matrix4 &model, float width, const floats4 &color);
  void drawRectangle(const matrix4 &model, rynx::graphics::texture_id tex_id,
                     const floats4 &color = floats4(1, 1, 1, 1));

  void drawMesh(const mesh &mesh, rynx::graphics::texture_id texture_id,
                const matrix4 &model,
                const floats4 &color = floats4(1, 1, 1, 1));
  void drawMesh(mesh_id id, rynx::graphics::texture_id texture_id,
                const matrix4 &model,
                const floats4 &color = floats4(1, 1, 1, 1)) {
  ...
  void
  drawMeshInstanced(const mesh &mesh, const std::vector<matrix4> &models,
                    const std::vector<floats4> &colors,
                    const std::vector<rynx::graphics::texture_id> &tex_data) {
    instanced_draw_impl(mesh, models.size(), models.data(), colors.data(),
                        tex_data.data(), nullptr, DrawType::Forward);
  }

  void
  drawMeshInstanced(mesh_id id, const std::vector<matrix4> &models,
                    const std::vector<floats4> &colors,
                    const std::vector<rynx::graphics::texture_id> &tex_data) {
  ...
  void drawMeshInstancedDeferred(
      const mesh &mesh, const std::vector<matrix4> &models,
      const std::vector<floats4> &colors,
      const std::vector<rynx::graphics::texture_id> &tex_data) {
    instanced_draw_impl(mesh, models.size(), models.data(), colors.data(),
                        tex_data.data(), nullptr, DrawType::Deferred);
  }

  void drawMeshInstancedDeferred(
      mesh_id id, const std::vector<matrix4> &models,
      const std::vector<floats4> &colors,
      const std::vector<rynx::graphics::texture_id> &tex_data) {
  ...
  void drawMeshInstancedDeferred(
      const mesh &mesh, size_t num, const matrix4 *models,
      const floats4 *colors, const rynx::graphics::texture_id *tex_data,
      const uint8_t *opaqueness) {
    instanced_draw_impl(mesh, num, models, colors, tex_data, opaqueness,
                        DrawType::Deferred);
  }

...
rynx::graphics::shader::shader(rynx::string name,
                               const char *vertexShaderSource,
                               const char *fragmentShaderSource)
    : m_name(std::move(name)), m_programID(0), m_vertexID(0), m_geometryID(0),
      m_fragmentID(0), m_started(false) {
  m_vertexID = compileVertexShader(vertexShaderSource);
  printlogmsg(m_vertexID);
  m_fragmentID = compileFragmentShader(fragmentShaderSource);
  printlogmsg(m_fragmentID);
...
rynx::graphics::shader::shader(rynx::string name, const char *vert,
                               const char *frag, const char *geom, GLint input,
...
GLint rynx::graphics::shader::uniform(const rynx::string &name) {
  ...
  rynx_assert(
      m_started,
      "looking for uniform when shader has not been started! not good.");
  auto it = m_uniformLocations.find(name);
  if (it == m_uniformLocations.end()) {
    GLint location = glGetUniformLocation(m_programID, name.c_str());
    m_uniformLocations.emplace(name, location);
    rynx_assert(location != -1, "shader has no uniform with name '%s'",
                name.c_str());
    return location;
  } else {
    return it->second;
...
rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, float value) {
  glUniform1f(uniform(name), value);
  return *this;
}

rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, float value1,
                                float value2) {
...
rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, int32_t value) {
  glUniform1i(uniform(name), value);
  return *this;
}

rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, int32_t value1,
                                int32_t value2) {
...
rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name,
                                const rynx::matrix4 &matrix) {
  glUniformMatrix4fv(uniform(name), 1, false, matrix.m);
  return *this;
}

rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, floats4 *v,
                                size_t amount) {
  static_assert(sizeof(floats4) == 4 * sizeof(float),
                "can't take array of floats4 directly because of padding.");
...
// transmitting them to gpu we agree that they are of type vec4 from gpu
// perspective.
rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, vec3f *v,
                                size_t amount) {
  static_assert(sizeof(vec3f) == 4 * sizeof(float),
                "can't take array of vec3f directly because of padding.");
  glUniform4fv(uniform(name), GLsizei(amount), reinterpret_cast<float *>(v));
  return *this;
}

rynx::graphics::shader &
rynx::graphics::shader::uniform(const rynx::string &name, float *v,
                                size_t amount) {
...
GLint rynx::graphics::shader::attribute(const rynx::string &name) {
  rynx_assert(m_started,
              "trying to fetch attribute from a shader that is not active!");
  auto it = m_uniformLocations.find(name);
  if (it == m_uniformLocations.end()) {
    GLint location = glGetAttribLocation(m_programID, name.c_str());
    m_uniformLocations.emplace(name, location);
    rynx_assert(location != -1,
                "WARNING: shader has no attribute with name '%s'",
                name.c_str());
    return location;
  } else {
    return it->second;
...
GLint rynx::graphics::shader::attribute_optional(const rynx::string &name) {
  rynx_assert(m_started,
              "trying to fetch attribute from a shader that is not active!");
  auto it = m_uniformLocations.find(name);
  if (it == m_uniformLocations.end()) {
...
void rynx::graphics::shader::stop() { m_started = false; }

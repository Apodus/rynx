...
struct motion {
  ...
  rynx::vec3<float> velocity_at_point(rynx::vec3<float> relative_point) const {
    return velocity + angularVelocity *
                          vec3<float>(-relative_point.y, +relative_point.x, 0);
  }

  rynx::vec3<float> velocity_at_point_predict(rynx::vec3<float> relative_point,
                                              float dt) const {
    return velocity + acceleration * dt +
           (angularVelocity + dt * angularAcceleration) *
               rynx::vec3<float>(-relative_point.y, +relative_point.x, 0);
  }
};
...
struct lifetime {
  ...
  operator float() const { return linear(); }

  float operator()() const { return linear(); }

  // 0 -> 1
  float linear() const { return 1.0f - value / max_value; }

  // 1 -> 0
  float linear_inv() const { return 1.0f - linear(); }
  ...
  float quadratic_inv() const { return 1.0f - quadratic(); }
  ...
  // square of the inverted linear, inverted. gives similar round shape as
  // quadratic. just goes from 0 to 1 with a steep rise and slows down towards
  // the end.
...
namespace phys {
struct body {
  body(uint64_t collision_id = 0) : collision_id(collision_id) {}
  ...
  float ANNOTATE(">=0") bias_multiply =
      1.0f; // how strongly this body rejects other bodies. static terrain
            // should have higher value than your basic dynamic object.
  float ANNOTATE(">=0") ANNOTATE("inverse") inv_mass = 0.100f; // 10kg default
  float ANNOTATE(">=0") ANNOTATE("inverse") inv_moment_of_inertia =
      0.0002f; // Should be computed from mass + shape kgm^2 ?
  float ANNOTATE("range 0 1") collision_elasticity = 0.5f; // [0, 1[
  float ANNOTATE(">=0") friction_multiplier = 1.0f;        // [0, 1]
  uint64_t collision_id = 0; // if two colliding objects have the same collision
                             // id (!= 0) then the collision is ignored.
};

struct boundary {
  boundary() {}
  boundary(rynx::polygon b, rynx::vec3f pos = rynx::vec3f(), float angle = 0.0f)
      : segments_local(std::move(b)) {
  ...
  void update_world_positions(rynx::vec3f pos, float angle) {
...

    {
      ...
      for (size_t i = 0; i < num_segments; ++i) {
        rynx::vec3f new_position =
            math::rotatedXY(segments_local.vertex_position(i), sin_v, cos_v) +
            pos;
        rynx::vec3f new_segment_normal =
            math::rotatedXY(segments_local.segment_normal(i), sin_v, cos_v);
        rynx::vec3f new_vertex_normal =
            math::rotatedXY(segments_local.vertex_normal(i), sin_v, cos_v);
        editor.vertex(int(i)).set(new_position, new_segment_normal,
                                  new_vertex_normal);
      }
    }

    rynx_assert(segments_local.size() == num_segments,
                "boundary edited during update");
  }
...
struct light_omni {
  ...
  floats4 ANNOTATE("rename x red") ANNOTATE("rename y green")
      ANNOTATE("rename z blue") ANNOTATE("rename w luminance")
          ANNOTATE("applies_to x y z") ANNOTATE("range 0 1")
              ANNOTATE("applies_to w") ANNOTATE(">=0") color;

  // light strength at point is clamp(0, 1, object normal dot light direction +
  // ambient) / sqr(dist) when ambient value = 0, the backside of objects are
  // not illuminated. when ambient value = 1, objects are evenly illuminated.
  float ANNOTATE("range 0 1") ambient = 0;

  float ANNOTATE(">=0") attenuation_linear = 0.1f;
  float ANNOTATE(">=0") attenuation_quadratic = 0.002f;
};
...
struct opaqueness {
  opaqueness() {}
  opaqueness(uint8_t value) : value(value) {}
  uint8_t ANNOTATE("range 0 255") value = 255;
};
} // namespace graphics

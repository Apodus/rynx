...
using DefaultProcessingFunctionality =
    rynx::application::Application::default_frame_processing_functionality;

DefaultProcessingFunctionality::default_frame_processing_functionality(
    rynx::observer_ptr<Application> host)
    : m_host(host) {
...
void DefaultProcessingFunctionality::menu_input() {
  auto &menuSystem =
      m_host->simulation_context()->get_resource<rynx::menu::System>();
  rynx::mapped_input &gameInput =
      m_host->simulation_context()->get_resource<rynx::mapped_input>();
...
void DefaultProcessingFunctionality::logic_frame_start() {
  ...
  m_dt = std::clamp(m_wall_clock_frame_timer.time_since_begin_us() * 0.001f *
                        0.001f,
                    0.0001f, 0.016f);
...
void DefaultProcessingFunctionality::logic_frame_wait_end() {
  ...
  // logic_time.observe_value(swap_time.avg() + logic_time_us / 1000.0f); //
...
void DefaultProcessingFunctionality::render() {
  if (true) {
...

    {
      ...
      render.light_global_directed({1.0f, 1.0f, 1.0f, 1.0f},
                                   {1.0f, 0.0f, 0.0f});
      ...
      // while waiting for computing to be completed, draw menus.
      {
        ...
        // 2, 2 is the size of the entire screen (in case of 1:1 aspect ratio)
        // for menu camera. left edge is [-1, 0], top right is [+1, +1], etc. so
        // we make it size 2,2 to cover all of that. and then take aspect ratio
        // into account by dividing the y-size.
        ...
        auto get_min_avg_max = [](rynx::numeric_property<float> &prop) {
          return rynx::to_string(prop.min()) + "/" +
                 rynx::to_string(prop.avg()) + "/" +
                 rynx::to_string(prop.max()) + "ms";
        };
      }
...
void rynx::application::Application::openWindow(int width, int height,
                                                rynx::string name) {
  ...
  m_shaders =
      rynx::make_shared<rynx::graphics::shaders>(m_simulation.m_vfs.get());
  m_renderer =
      rynx::make_shared<rynx::graphics::renderer>(m_textures, m_shaders);
  m_debugVisualization =
      rynx::make_shared<application::DebugVisualization>(m_renderer);

  this->simulation_context()->set_resource(m_camera);
  m_rendering_steps = rynx::make_unique<rynx::application::renderer>(
      textures(), shaders(), renderer(),
...
rynx::application::Application::current_window_size() const {
  return {m_window->width(), m_window->height()};
}

void rynx::application::Application::on_resize(
    rynx::function<void(size_t, size_t)> onWindowResize) {
...
float rynx::application::Application::aspectRatio() const {
  return m_window->getAspectRatio();
}
bool rynx::application::Application::isExitRequested() const {
  return m_window->shouldClose();
}

void rynx::application::Application::set_gl_viewport_to_window_dimensions()
    const {

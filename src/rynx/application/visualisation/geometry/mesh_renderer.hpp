...
private:
  static_assert(sizeof(rynx::components::transform::matrix) ==
                    sizeof(rynx::matrix4),
                "matrices need to be in consecutive memory");
...
public:
  ...
  virtual void prepare(rynx::scheduler::context *ctx) override {
    ctx->add_task("model matrices", [this](rynx::ecs &ecs) mutable {
      ...
      ecs.query()
          .notIn<rynx::components::graphics::translucent,
                 components::graphics::frustum_culled,
          ...
          .for_each_buffer(
              [this](size_t num_entities,
                ...
                m_bufs.emplace_back(buffer{num_entities, mesh, positions, radii,
                                           colors, models, tex_ids, nullptr});
              });
  ...
  virtual void execute() override {
    std::sort(m_bufs.begin(), m_bufs.end(),
              [](const auto &a, const auto &b) -> bool {
                return a.positions[0].value.z < b.positions[0].value.z;
              });
    for (auto &&buf : m_bufs) {
      m_meshRenderer->drawMeshInstancedDeferred(
          *buf.mesh, buf.num,
          reinterpret_cast<const rynx::matrix4 *>(buf.models),
          reinterpret_cast<const floats4 *>(buf.colors), buf.tex_ids,

...
bool rynx::input::modifiers_t::shift_left() {
  return m_host->isKeyDown(rynx::key::codes::shift_left(), true);
}
bool rynx::input::modifiers_t::shift_right() {
  return m_host->isKeyDown(rynx::key::codes::shift_right(), true);
}

bool rynx::input::modifiers_t::alt_left() {
  return m_host->isKeyDown(rynx::key::codes::alt_left(), true);
}
bool rynx::input::modifiers_t::alt_right() {
  return m_host->isKeyDown(rynx::key::codes::alt_right(), true);
}

bool rynx::input::modifiers_t::ctrl_left() {
  return m_host->isKeyDown(rynx::key::codes::control_left(), true);
}
bool rynx::input::modifiers_t::ctrl_right() {
  return m_host->isKeyDown(rynx::key::codes::control_right(), true);
}

rynx::input::input(rynx::shared_ptr<Window> window) {
  rynx_assert(!g_keyboardKeyEventHandler,
              "Multiple UserIO initialisations not allowed :( Never fix");
  ...
  g_keyboardKeyEventHandler = [this](int key, int scancode, int action,
                                     int mods) {
    onKeyEvent(key, scancode, action, mods);
  };
  g_mouseScrollEventHandler = [this](double /* xoffset */, double yoffset) {
    onMouseScrollEvent(0, yoffset);
  };
  g_mouseMoveEventHandler = [this](double xpos, double ypos) {
    onMouseMoveEvent(xpos, ypos);
  };
  g_mouseKeyEventHandler = [this](int key, int action, int mods) {
    onMouseButtonEvent(key, action, mods);
  };
...
bool rynx::input::isKeyValueInState(int8_t keyValue, int8_t requestedValue,
                                    bool ignoreConsumed) {
  return (keyValue &
          (requestedValue | (!ignoreConsumed * rynx::input::KEY_CONSUMED))) ==
         requestedValue;
}

bool rynx::input::isKeyClicked(rynx::key::physical key,
                               bool ignoreConsumed) const {
  return isKeyInState(key, KEY_CLICK, ignoreConsumed);
}

bool rynx::input::isKeyPressed(rynx::key::physical key,
                               bool ignoreConsumed) const {
  return isKeyInState(key, KEY_PRESSED, ignoreConsumed);
}

bool rynx::input::isKeyDown(rynx::key::physical key,
                            bool ignoreConsumed) const {
  return isKeyInState(key, KEY_DOWN, ignoreConsumed);
}

bool rynx::input::isKeyRepeat(rynx::key::physical key,
                              bool ignoreConsumed) const {
  return isKeyInState(key, KEY_REPEAT, ignoreConsumed);
}

bool rynx::input::isKeyReleased(rynx::key::physical key,
                                bool ignoreConsumed) const {
...
float rynx::input::getMouseScroll() const { return m_mouseScroll; }
...
void rynx::input::update() {
  for (uint8_t &keyState : m_buttonStates) {
    keyState &=
        ~(KEY_PRESSED | KEY_RELEASED | KEY_REPEAT | KEY_CLICK | KEY_CONSUMED);
  }
  m_mouseScroll = 0;
...
void rynx::input::onKeyEvent(int key, int /* scancode */, int action,
                             int /* mods */) {
  if (key < 0)
    return;
...
rynx::key::physical rynx::input::getMouseKeyCode(int mouseButton) const {
  rynx_assert(mouseButton < ::sMaxMouseButtons,
              "not that many mouse buttons are supported");
  return {mouseButton + 1 + GLFW_KEY_LAST};
}

rynx::key::physical rynx::input::getMidiKeyCode(int midiButton) const {
  static_assert(::sMaxMouseButtons + GLFW_KEY_LAST + 1 + 128 < 512,
                "not enough space for midi keys");
  rynx_assert(midiButton + ::sMaxMouseButtons + GLFW_KEY_LAST + 1 <
                  m_buttonStates.size(),
              "not that many midi buttons are supported");
...
void rynx::input::onMouseButtonEvent(int key, int action, int /* mods */) {
  ...
  if (action == GLFW_RELEASE) {
    ...
    if ((m_mousePosition - m_mousePosition_clickBegin).length_squared() <
        0.02f * 0.02f) {
      value |= KEY_CLICK;
    }
...
void rynx::input::onMouseMoveEvent(double xpos, double ypos) {
  vec3f newPos(2.0f * static_cast<float>(xpos) / m_window->width() - 1.0f,
               1.0f - 2.0f * static_cast<float>(ypos) / m_window->height(), 0);
...
void rynx::input::keyCallbackDummy(GLFWwindow * /* window */, int key,
                                   int scancode, int action, int mods) {
  if (g_keyboardKeyEventHandler)
    g_keyboardKeyEventHandler(key, scancode, action, mods);
}

void rynx::input::mouseButtonCallbackDummy(GLFWwindow * /* window */,
                                           int button, int action, int mods) {
  if (g_mouseKeyEventHandler)
    g_mouseKeyEventHandler(button, action, mods);
}

void rynx::input::scrollCallbackDummy(GLFWwindow * /* window */, double xoffset,
                                      double yoffset) {
  if (g_mouseScrollEventHandler)
    g_mouseScrollEventHandler(xoffset, yoffset);
}

void rynx::input::cursorPosCallbackDummy(GLFWwindow * /* window */, double xpos,
                                         double ypos) {
  if (g_mouseMoveEventHandler)
    g_mouseMoveEventHandler(xpos, ypos);
}

void rynx::input::cursorEnterCallbackDummy(GLFWwindow * /* window */,
                                           int entered) {
  if (g_mouseEnteredHandler)
    g_mouseEnteredHandler(entered);
}

rynx::scoped_input_inhibitor rynx::input::inhibit_mouse_scoped() {
  return scoped_input_inhibitor(&m_inhibited, false, true);
}
rynx::scoped_input_inhibitor rynx::input::inhibit_keyboard_scoped() {
  return scoped_input_inhibitor(&m_inhibited, true, false);
}
rynx::scoped_input_inhibitor rynx::input::inhibit_mouse_and_keyboard_scoped() {
  return scoped_input_inhibitor(&m_inhibited, true, true);
}

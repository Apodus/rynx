...
class InputDLL input {
  ...
  bool queueMidiEvent(uint8_t status, uint8_t key, uint8_t velocity) {
    ...
    */
    ...
    m_midiEventQueue[m_midiEventQueueWrite] = {status, key, velocity,
                                               uint8_t()};
    m_midiEventQueueWrite =
        (m_midiEventQueueWrite + 1) % m_midiEventQueue.size();
    return true;
  }

  void processMidiEventQueue() {
    while ((m_midiEventQueueRead + 1) % m_midiEventQueue.size() !=
           m_midiEventQueueWrite) {
      m_midiEventQueueRead =
          (m_midiEventQueueRead + 1) % m_midiEventQueue.size();
      auto midi_event = m_midiEventQueue[m_midiEventQueueRead];
      if (midi_event.status == 0x90) {
        m_buttonStates[getMidiKeyCode(midi_event.key).id] |=
            KEY_PRESSED | KEY_DOWN;
      } else if (midi_event.status == 0x80) {
        m_buttonStates[getMidiKeyCode(midi_event.key).id] =
            KEY_RELEASED | KEY_CLICK;
      }
    }
  }
  ...
  int32_t isInhibited(rynx::key::physical key) const {
    int32_t keyboardInhibited =
        ((m_inhibited.keyboard > 0) * (key.id < getMouseKeyCode(0).id));
    int32_t mouseInhibited =
        ((m_inhibited.mouse > 0) * (key.id >= getMouseKeyCode(0).id));
  ...
  static bool isKeyValueInState(int8_t keyState, int8_t requestedState,
                                bool ignoreConsumed);
  inline bool isKeyInState(rynx::key::physical key, int8_t requestedState,
                           bool ignoreConsumed) const {
    return isUnhinged(key) * isKeyValueInState(m_buttonStates[key.id],
                                               requestedState, ignoreConsumed);
  }
...
public:
  ...
  rynx::key::physical getMouseKeyCode(int mouseButton) const;
  rynx::key::physical getMidiKeyCode(int midiButton) const;

  template <class T> inline void getCursorPosition(T &t) const {
    t.x = m_mousePosition.x;
    t.y = m_mousePosition.y;
  }

  vec3<float> getCursorPosition() const { return m_mousePosition; }

  vec3<float> getCursorDelta() const { return m_mouseDelta; }
  ...
  bool isKeyReleased(rynx::key::physical key,
                     bool ignoreConsumed = false) const;
  ...
  static void keyCallbackDummy(GLFWwindow *window, int key, int scancode,
                               int action, int mods);
  static void mouseButtonCallbackDummy(GLFWwindow *window, int button,
                                       int action, int mods);
  static void scrollCallbackDummy(GLFWwindow *window, double xoffset,
                                  double yoffset);
  static void cursorPosCallbackDummy(GLFWwindow *window, double xpos,
                                     double ypos);
...
public:
  ...
  scoped_input_inhibitor(input::inhibit_state *host, bool keyboard, bool mouse)
      : host(host), inhibit_mouse(mouse), inhibit_keyboard(keyboard) {
    if (inhibit_mouse) {
      ++host->mouse;
    }
    if (inhibit_keyboard) {

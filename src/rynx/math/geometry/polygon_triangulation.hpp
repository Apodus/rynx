...
namespace rynx {
	...
	public:
		...
		std::unique_ptr<mesh> make_mesh(const rynx::polygon& polygon_, floats4 uvLimits = floats4(0.0f, 0.0f, 1.0f, 1.0f));
		std::unique_ptr<mesh> make_boundary_mesh(const rynx::polygon& polygon_, floats4 texCoords, float line_width = 1.0f);
	};
}
...
TEST_CASE("serialization", "strings & vectors") {
  ...
  std::vector<std::vector<rynx::string>> moreStrings{
      {"yks"}, {"kaks"}, {"kolme", "kolme ja puol"}};
  ...
  auto result2 =
      rynx::deserialize<std::vector<std::vector<rynx::string>>>(reader);
  REQUIRE(moreStrings == result2);
}

struct hubbabubba {
  bool operator==(const hubbabubba &other) const {
    return kekkonen == other.kekkonen && lol == other.lol;
  }
...
template <> struct Serialize<hubbabubba> {
  ...
  template <typename IOStream> void deserialize(hubbabubba &a, IOStream &io) {
...
template <> struct Serialize<id_field_t> {
  ...
  template <typename IOStream> void deserialize(id_field_t &a, IOStream &io) {
    rynx::deserialize(a.target, io);
  }
};

template <> struct for_each_id_field_t<id_field_t> {
  template <typename Op> void execute(id_field_t &t, Op &&op) {
...
TEST_CASE("serialize ecs with id field", "serialization") {
  ...
  rynx::serialization::vector_writer out =
      rynx::ecs_detail::raw_serializer(a).serialize(reflections);
...

  {
    ...
    // NOTE: Raw serialization does not correct the id-links to point to newly
    // created entities.
...
TEST_CASE("serialize ecs with primitives", "serialization") {
...

  {
    ...
    b.query().for_each([](int f) { REQUIRE(f == 1); });
  }
...
TEST_CASE("ecs views usable", "verify compiles") {
  // these must compile. that is all.
  {
    ...
    view.query().for_each(
        [](const rynx::components::transform::position &) {}); // is ok.
    view.query().for_each(
        [](rynx::components::transform::position &) {}); // is ok.
  }

  {
    rynx::ecs ecs;
    rynx::ecs::view<rynx::components::transform::position> mutable_view = ecs;
    rynx::ecs::view<const rynx::components::transform::position> view =
        mutable_view; // more strict view can be constructed from another.
    view.query().for_each(
        [](const rynx::components::transform::position &) {}); // is ok.
    // view.query().for_each([](rynx::components::transform::position&) {}); //
    // error C2338:  You promised to access this type in only const mode!
    // view.query().for_each([](rynx::components::transform::radius&) {}); //
    // error C2338:  You have promised to not access this type in your ecs view.
  }
}
...
TEST_CASE("ecs range for_each") {
  ...
  int counter = 0;
  auto stop_point = ecs.query().for_each_partial(rynx::ecs::range{0, 50},
                                                 [&](int a) { counter += a; });

  counter = 0;
  stop_point =
      ecs.query().for_each_partial(stop_point, [&](int a) { counter += a; });
  ...
  for (int i = 1; i < 101; ++i) {
    ...
    counter = 0;
    while (1) {
      iter = ecs.query().for_each_partial(iter, [&](int a) { counter += a; });
...
TEST_CASE("ecs for_each iterates correct amount of entities") {
  {
...

    {
      ...
      view.query().for_each([&count](a &) { ++count; });
...

    {
      ...
      view.query().for_each([&count](b &) { ++count; });
...

    {
      ...
      view.query().for_each([&count](a &, b &) { ++count; });
...
struct mydata : public rynx::ecs_value_segregated_component_tag {
  mydata() {}
  mydata(int v) : data(v) {}
  bool operator==(const mydata &other) const { return data == other.data; }
...
template <> struct Serialize<mydata> {
  template <typename IOStream> void serialize(const mydata &, IOStream &) {}

  template <typename IOStream> void deserialize(mydata &, IOStream &) {}
};
} // namespace rynx::serialization
...
// you can run the same benchmarks against entt if you want. find out how to
// setup entt from their github pages.
namespace entt {
class registry;
}
...
template <typename... Ts> int64_t ecs_for_each(rynx::ecs &ecs) {
  ...
  ecs.query().for_each(
      [&](Ts... ts) { count += (static_cast<int64_t>(ts) + ...); });
  return count;
}
...
#include <rynx/ecs/blocked_table.hpp>
#include <rynx/tech/memory_block_pool.hpp>


TEST_CASE("rynx tech: memory_block_pool") {
  rynx::tech::memory_block_pool pool(1024); // 1KB blocks

  void *b1 = pool.acquire();
  void *b2 = pool.acquire();
  void *b3 = pool.acquire();

  REQUIRE(pool.total_allocated_blocks() == 3);
  ...
  pool.release(b3);
  pool.release(b4);
  REQUIRE(pool.free_blocks() == 3);
}

TEST_CASE("rynx ecs internal: blocked table") {
  rynx::tech::memory_block_pool pool(1024);

  struct TestComp {
    int a, b;
    TestComp(int a, int b) : a(a), b(b) {}
  };

  rynx::ecs_internal::blocked_table<TestComp> table(&pool);
  REQUIRE(table.size() == 0);

  for (int i = 0; i < 1000; ++i) {
    table.emplace_back(i, i * 2);
  }

  REQUIRE(table.size() == 1000);
  REQUIRE(pool.total_allocated_blocks() > 0);

  for (int i = 0; i < 1000; ++i) {
    REQUIRE(table[i].a == i);
    REQUIRE(table[i].b == i * 2);
  }

  size_t initial_allocations = pool.total_allocated_blocks();

  // test replace with back and pop
  table.replace_with_back_and_pop(500);
  REQUIRE(table.size() == 999);
  REQUIRE(table[500].a == 999);
  REQUIRE(table[500].b == 999 * 2);

  // test push_back
  TestComp t2{123, 456};
  table.push_back(t2);
  REQUIRE(table.size() == 1000);
  REQUIRE(table.back().a == 123);

  // test operator[]
  REQUIRE(table[999].a == 123);
  ...
  REQUIRE(table.size() == 0);
  REQUIRE(pool.free_blocks() == initial_allocations);
}

...
uniform vec4 lights_directions[64]; // xyz = direction, w = arc (0-1, <0.0 = omni)
...
void main()
{
	...
	for(int i=0; i<lights_num; ++i) {
		...
		result += shadow_factor * lighting_global_mul * fragment_color_intensity / (distance_sqr * lights_settings[i].z + sqrt(distance_sqr) * lights_settings[i].y + 1.0);
	}
